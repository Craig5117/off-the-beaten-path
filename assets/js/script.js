      ...
      response.json().then(function (data) {
        ...
        for (var i = 0; i < hourlyForecast.length; ++i) {
          ...
          if (byHour === 0) {
            ...
            spanEl.textContent =
              "At " + forecastHour + " expect " + forecastCondition + ".";
          } else if (byHour === 1) {
    ...
    if (response.ok) {
      response.json().then(function (data) {
        ...
        if (data.results[0].address.municipality) {
          resultPlaceName =
            resultPlaceName + data.results[0].address.municipality + " ";
        }
        if (data.results[0].address.countrySubdivision) {
          resultPlaceName =
            resultPlaceName + data.results[0].address.countrySubdivision + " ";
        }
        if (data.results[0].address.countryCode) {
          resultPlaceName =
            resultPlaceName + data.results[0].address.countryCode;
        }
          ...
          if (response.ok) {
            response.json().then(function (data) {
              if (data.trails.length === 0) {
                ...
                trailDivEl.classList =
                  "trailDiv box has-background-danger-dark has-text-white";
                var noTrailsEl = document.createElement("p");
                noTrailsEl.textContent =
                ...
                for (var i = 0; i < data.trails.length; ++i) {
                  ...
                  var trailDivEl = document.createElement("div");
                  trailDivEl.classList =
                    "trailDiv box has-background-danger-dark has-text-white mb-0";
                  trailDivEl.setAttribute("id", trName);
                  var ulEl = document.createElement("ul");
                  var trNameLiEl = document.createElement("li");
                  trNameLiEl.innerHTML =
                    "<span id='thisTrName' class='is-size-4'>" +
                    trName +
                    "</span>";
                  ...
                  latliEl.innerHTML =
                    "Lat: <span id='thisTrLat'>" +
                    trLat +
                    "</span> Lon: <span id='thisTrLon'>" +
                    trLon +
                    "</span>";
                  ...
                  instructionsEL.innerHTML =
                    "Click on this red info box to see an <span class='has-text-weight-bold'>8 Hour weather forecast</span> for this trail.";
                  ...
                  direliEl.innerHTML =
                    "<a href='" +
                    directions +
                    "' target='_blank' class='has-text-white has-text-weight-bold'>Directions</a>";
                  ...
                  infoliEl.innerHTML =
                    "<a href='" +
                    moreInformation +
                    "' target='_blank' class='has-text-white has-text-weight-bold'>More Information</a>";
                  var dirInfoContainer = document.createElement("div");
                  dirInfoContainer.classList =
                    "dirInfoLink is-flex is-justify-content-space-between mt-2";
      ...
      var noTrailsEl = document.createElement("p");
      noTrailsEl.textContent =
        "We are having trouble displaying information for that location. Check your internect connection and please try again.";
      trailDivEl.append(noTrailsEl);
      trailSectionEl.append(trailDivEl);
    }
  });
...
var loadHistory = function () {
  ...
  if (!locationHistory) {
    ...
    for (var i = 0; i < locationHistory.length; ++i) {
      ...
      historyEl.classList =
        "search-history button p-3 is-clipped has-background-dark has-text-white has-text-left";
